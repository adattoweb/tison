...
export const Input = forwardRef<HTMLInputElement, InputProps>(
   ({ Icon, label, placeholder = "", className, type = "text", ...props }, ref) => {
      return (
         <div className={clsx(className, "flex flex-col gap-1.5 w-full")}>
            <p className="text-base font-medium">{label}</p>
            <div className="flex items-center gap-2 border border-(--stroke-color) focus-within:border-(--stroke-light-color) rounded-md bg-(--bg-trans-color) py-2 px-2.5 h-11">
