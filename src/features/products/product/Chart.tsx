...
export function Chart({ className, productModelId, dateFrom, dateTo }: ChartProps) {
   const { data, isLoading, isError } = useDefectsByProductModel(productModelId, { dateFrom, dateTo })

   const defects: DefectItem[] = useMemo(
      () =>
         (data ?? [])
            .filter(row => row.defects_count > 0)
            .map(row => ({
               name: row.name,
               value: row.defects_count,
               fill: row.color,
            })),
      [data],
   )
   ...
   return (
      ...
      >
         ...
         {isError && <p className="text-red-400">Не вдалося завантажити дані про дефекти</p>}
